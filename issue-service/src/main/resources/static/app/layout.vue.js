...
`,
    ...
    methods: {
        fetchIssues(_status) {
            ...
            window.axios.get(`/api/issues?status=${status}`, {
                headers: {
        ...
        fetchUser() {
            ...
            window.axios.get(`${url}/api/users/me`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`
                }
            }).then(response => {
        ...
        fetchReporter(issue) {
            let accessToken = localStorage.getItem('token')
            let url = localStorage.getItem('userServiceUrl')
            window.axios.get(`${url}/api/users/${issue.userId}/username`,
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    }
        ...
        logout() {
            let accessToken = localStorage.getItem('token')
            let url = localStorage.getItem('userServiceUrl')
            window.axios.delete(`${url}/api/users/logout`,
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    }
